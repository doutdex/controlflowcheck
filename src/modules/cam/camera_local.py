...
def start_camera(page: ft.Page, target_container: Optional[ft.Control] = None):
    # Global components
    ...
    if target_container is None:
        ...
        try:
            if hasattr(target_container, "controls"):
                # Works for Column, Row, Tabs, etc.
                target_container.controls.clear()
                target_container.controls.append(tabs)
            else:
                target_container.content = tabs
        except AttributeError:
            # Fallback: attempt to set `content` if available
            setattr(target_container, "content", tabs)
